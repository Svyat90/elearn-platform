...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.agentMetum.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.agent-meta.update", [$agentMetum->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ ($agentMetum->user ? $agentMetum->user->id : old('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('agent_commission') ? 'is-invalid' : '' }}" type="number" name="agent_commission" id="agent_commission" value="{{ old('agent_commission', $agentMetum->agent_commission) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('state') ? 'is-invalid' : '' }}" type="text" name="state" id="state" value="{{ old('state', $agentMetum->state) }}">
                @if($errors->has('state'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('city') ? 'is-invalid' : '' }}" type="text" name="city" id="city" value="{{ old('city', $agentMetum->city) }}">
                @if($errors->has('city'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('agent_status') ? 'is-invalid' : '' }}" type="text" name="agent_status" id="agent_status" value="{{ old('agent_status', $agentMetum->agent_status) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('registered_on') ? 'is-invalid' : '' }}" type="text" name="registered_on" id="registered_on" value="{{ old('registered_on', $agentMetum->registered_on) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('agent') ? 'is-invalid' : '' }}" name="agent_id" id="agent_id">
                    @foreach($agents as $id => $agent)
                        <option value="{{ $id }}" {{ ($agentMetum->agent ? $agentMetum->agent->id : old('agent_id')) == $id ? 'selected' : '' }}>{{ $agent }}</option>
                    @endforeach
                </select>
                @if($errors->has('agent'))
