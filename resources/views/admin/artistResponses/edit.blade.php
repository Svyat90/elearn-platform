...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.artistResponse.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.artist-responses.update", [$artistResponse->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('order') ? 'is-invalid' : '' }}" name="order_id" id="order_id">
                    @foreach($orders as $id => $order)
                        <option value="{{ $id }}" {{ ($artistResponse->order ? $artistResponse->order->id : old('order_id')) == $id ? 'selected' : '' }}>{{ $order }}</option>
                    @endforeach
                </select>
                @if($errors->has('order'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('artist_action') ? 'is-invalid' : '' }}" name="artist_action" id="artist_action">
                    ...
                    @foreach(App\ArtistResponse::ARTIST_ACTION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('artist_action', $artistResponse->artist_action) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('video_status') ? 'is-invalid' : '' }}" name="video_status" id="video_status">
                    ...
                    @foreach(App\ArtistResponse::VIDEO_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('video_status', $artistResponse->video_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('video') ? 'is-invalid' : '' }}" name="video_id" id="video_id">
                    @foreach($videos as $id => $video)
                        <option value="{{ $id }}" {{ ($artistResponse->video ? $artistResponse->video->id : old('video_id')) == $id ? 'selected' : '' }}>{{ $video }}</option>
                    @endforeach
                </select>
                @if($errors->has('video'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('artist_note') ? 'is-invalid' : '' }}" type="text" name="artist_note" id="artist_note" value="{{ old('artist_note', $artistResponse->artist_note) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('action_update') ? 'is-invalid' : '' }}" type="text" name="action_update" id="action_update" value="{{ old('action_update', $artistResponse->action_update) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('completion_update') ? 'is-invalid' : '' }}" type="text" name="completion_update" id="completion_update" value="{{ old('completion_update', $artistResponse->completion_update) }}">
