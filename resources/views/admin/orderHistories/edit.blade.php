...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.orderHistory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.order-histories.update", [$orderHistory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ ($orderHistory->user ? $orderHistory->user->id : old('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('videos') ? 'is-invalid' : '' }}" name="videos[]" id="videos" multiple>
                    @foreach($videos as $id => $video)
                        <option value="{{ $id }}" {{ (in_array($id, old('videos', [])) || $orderHistory->videos->contains($id)) ? 'selected' : '' }}>{{ $video }}</option>
                    @endforeach
                </select>
