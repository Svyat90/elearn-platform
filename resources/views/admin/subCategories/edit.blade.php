...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sub-categories.update", [$subCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="color">{{ trans('cruds.subCategory.fields.color') }}</label>
                <input class="form-control {{ $errors->has('color') ? 'is-invalid' : '' }}" type="text" name="color" id="color" value="{{ old('color', $subCategory->color) }}">
                @if($errors->has('color'))
                    <span class="text-danger">{{ $errors->first('color') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.subCategory.fields.color_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent') ? 'is-invalid' : '' }}" name="parent_id" id="parent_id" required>
                    @foreach($parents as $id => $parent)
                        <option value="{{ $id }}" {{ ($subCategory->parent ? $subCategory->parent->id : old('parent_id')) == $id ? 'selected' : '' }}>{{ $parent }}</option>
                    @endforeach
                </select>
...
@if(isset($subCategory) && $subCategory->image)
      ...
      this.options.thumbnail.call(this, file, '{{ $subCategory->image->getUrl('thumb') }}')
