    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-reviews.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('review_text') ? 'is-invalid' : '' }}" name="review_text" id="review_text">{!! old('review_text') !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('stars') ? 'is-invalid' : '' }}" type="number" name="stars" id="stars" value="{{ old('stars', '1') }}" step="1">
                @if($errors->has('stars'))
            ...
            <div class="form-group">
                ...
                @foreach(App\UserReview::SHOW_VIDEO_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('show_video') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="show_video_{{ $key }}" name="show_video" value="{{ $key }}" {{ old('show_video', '1') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('review_apporval') ? 'is-invalid' : '' }}" name="review_apporval" id="review_apporval">
                    ...
                    @foreach(App\UserReview::REVIEW_APPORVAL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('review_apporval', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
