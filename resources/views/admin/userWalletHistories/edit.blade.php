...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.userWalletHistory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-wallet-histories.update", [$userWalletHistory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('txn_type') ? 'is-invalid' : '' }}" name="txn_type" id="txn_type">
                    ...
                    @foreach(App\UserWalletHistory::TXN_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('txn_type', $userWalletHistory->txn_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $userWalletHistory->amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('txn_info') ? 'is-invalid' : '' }}" type="text" name="txn_info" id="txn_info" value="{{ old('txn_info', $userWalletHistory->txn_info) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\UserWalletHistory::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $userWalletHistory->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ ($userWalletHistory->user ? $userWalletHistory->user->id : old('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('earn_from') ? 'is-invalid' : '' }}" name="earn_from_id" id="earn_from_id">
                    @foreach($earn_froms as $id => $earn_from)
                        <option value="{{ $id }}" {{ ($userWalletHistory->earn_from ? $userWalletHistory->earn_from->id : old('earn_from_id')) == $id ? 'selected' : '' }}>{{ $earn_from }}</option>
                    @endforeach
                </select>
